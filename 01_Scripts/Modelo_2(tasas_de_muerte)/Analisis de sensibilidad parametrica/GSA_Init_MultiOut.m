%% GSA_Init: initialize the variables used in the GSA computation for multi-output systems
%
% Usage:
%   pro = GSA_Init(pro)
%
...
function pro = GSA_Init_MultiOut(pro)
...
fE = cell2mat(fE_cell); % this will only work if all output generated by the function is numeric

% Store simulation results in the project structure array
pro.GSA.fE = fE;

% calculate the mean value of the first model outcome
pro.GSA.mfE = nanmean(fE(:,1));
...
pro.GSA.f0 = nanmean(pro.GSA.fE(:,1) - pro.GSA.mfE);

% calculate the total variance of the model outcomes
pro.GSA.D  = nanmean((pro.GSA.fE(:,1) - pro.GSA.mfE).^2) - pro.GSA.f0^2;

% approximate the error of the mean value
pro.GSA.ef0 = 1.96*sqrt(pro.GSA.D/sum(~isnan(pro.GSA.fE(:,1) - pro.GSA.mfE)));
