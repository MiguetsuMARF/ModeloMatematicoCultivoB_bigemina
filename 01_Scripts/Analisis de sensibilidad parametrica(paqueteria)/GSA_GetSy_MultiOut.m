%% GSA_GetSy: calculate the Sobol' sensitivity indices for multi-output systems
%
% Usage:
...
function [S, eS, pro] = GSA_GetSy_MultiOut(pro, iset, verbose)
        ...
        if isnan(pro.GSA.GSI(si))
            ...
            if isnan(pro.GSA.Di(si))
                ...
                if isnan(pro.GSA.Dmi(si))
                    ...
                    fH = cell2mat(fH_cell); % this will only work if all output generated by the function is numeric
                    ...
                    % section 2.4 as I
                    ff = (pro.GSA.fE(:,1) - pro.GSA.mfE).*(fH(:,1)-pro.GSA.mfE);
